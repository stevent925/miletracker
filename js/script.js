$(document).one('pageinit', function() {
    ...
    function showRuns() {
        ...
        if (runs != '' && runs != null) {
            for (var i = 0; i < runs.length; i++) {
                $('#stats').append(`<li class="ui-body-inherit ui-li-static"><strong>Date: </strong>${runs[i]["date"]}<br><strong>Distance: </strong>${runs[i]["miles"]}m
                <div class="controls"><a href="#edit" id="editLink" data-miles="${runs[i]["miles"]}" data-date="${runs[i]["date"]}">Edit</a> | <a href="#" id="deleteLink" 
                data-miles="${runs[i]["miles"]}" data-date="${runs[i]["date"]}" onclick="return confirm('Are you sure?')">Delete</a></li>`);
            }
    ...
    function addRun() {
        ...
        // Set stringified objected to local storage
